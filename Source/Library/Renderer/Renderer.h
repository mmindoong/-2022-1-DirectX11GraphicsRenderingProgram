...
namespace library
{
    ...
    private:
        ...
        ComPtr<ID3D11Buffer> m_cbLights; //Constant buffer containing lights' position and color
        Camera m_camera;
        XMMATRIX m_projection;

        std::unordered_map<std::wstring, std::shared_ptr<Renderable>> m_renderables;
        std::shared_ptr<PointLight> m_aPointLights[NUM_LIGHTS]; //Array of shared pointers to PointLight
